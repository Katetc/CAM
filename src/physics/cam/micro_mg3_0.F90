...
use wv_sat_methods, only: &
     ...
     qsat_water_all => wv_sat_qsat_water_all, &
     qsat_ice => wv_sat_qsat_ice, &
     qsat_ice_all => wv_sat_qsat_ice_all
...
subroutine micro_mg_tend ( &
  ...
  real(r8) :: esn(mgncol,nlev)    ! checking for RH after rain evap

  ! saturation vapor mixing ratios
  real(r8) :: qvl(mgncol,nlev)    ! liquid
  real(r8) :: qvi(mgncol,nlev)    ! ice
  real(r8) :: qvn(mgncol,nlev)    ! checking for RH after rain evap
  real(r8) :: qvn_tmp_liq(mgncol,nlev)    ! checking for RH after rain evap
  real(r8) :: qvn_tmp_ice(mgncol,nlev)    ! checking for RH after rain evap
  ...
  real(r8) :: faloutc(mgncol,nlev)
  real(r8) :: faloutnc(mgncol,nlev)
  real(r8) :: falouti(mgncol,nlev)
  real(r8) :: faloutni(mgncol,nlev)

  real(r8) :: faloutr(mgncol,nlev)
  real(r8) :: faloutnr(mgncol,nlev)
  real(r8) :: falouts(mgncol,nlev)
  real(r8) :: faloutns(mgncol,nlev)
  ...
  real(r8) :: faloutg(mgncol,nlev)
  real(r8) :: faloutng(mgncol,nlev)
  real(r8) :: faltndg
  real(r8) :: faltndng
  ...
  real(r8) :: qtmp(mgncol,nlev)
  real(r8) :: qtmp_ice_pre(mgncol,nlev)
  real(r8) :: ttmp(mgncol,nlev)
  ...
  !$acc data create(  mi0l, falouti, faloutni, faloutc, faloutnc, faloutr, faloutnr, &
  !$acc&              falouts, faloutns, faloutg, faloutng, qc, nc, qi, ni, qr, nr, qs, ns, &
  !$acc&              qg, ng, lcldm, icldm, cldm, qsfm, rho, dv, mu, sc, rhof, arn, rainrt, &
  !$acc&              nnuccd, nimax, mnuccd, precip_frac, minstsm, ninstsm, pgam, lamc, prc, &
  !$acc&              nprc, nprc1, qiic, niic, qcic, ncic, minstrf, ninstrf, minstgm, ninstgm, &
  !$acc&              qric, nric, lami, n0i, ngic, qgic, nsic, qsic, prci, nprci, lami, n0i, &
  !$acc&              lamr, n0r, lams, n0s, lamg, n0g, uns, unr, mnuccc, nnuccc, nnucct, &
  !$acc&              asn, agn, acn, ain, ajn, esl, qvl, esi, qvi, relhum, &
  !$acc&              mnudep, nnudep, ung, nsacwi, msacwi, nsagg, psacws, npsacws, pracs, npracs, &
  !$acc&              mnuccr, nnuccr, pra, npra, nragg, prai, nprai, bergs, vap_dep, &
  !$acc&              ice_sublim, nsubi, nsubc, berg, pgracs, prdg, ngracs, qmultg, &
  !$acc&              nmultg, qmultrg, nmultrg, psacr, psacwg, npsacwg, pgsacw, &
  !$acc&              nscng, pracg, npracg, pre, prds, qtmp, ttmp, nsubs, nsubr, nnuccri, &
  !$acc&              mnuccri, dum_2D, dum1_2D, dum2_2D, dum3_2D, qvn_tmp_liq, qvn_tmp_ice, &
  !$acc&              qtmp_ice_pre, esn, qvn, dumni, dumnc, dumng, dumg, dumns, dums, dumnr, &
  !$acc&              dumr, dumc, dumi, fc, fnc, fi, fni, fs, fns, fr, fnr, fg, fng, pdel_inv, &
  !$acc&              nstep, nstep_max, lamx_tmp, dumni0, pgam_tmp, lamc_tmp, dumns0 ) &
  !$acc&     copyin(  qcn, ncn, qin, nin, qrn, nrn, qsn, nsn, qgr, ngr, &
  !$acc&              qsatfac, p, t , q, npccn, naai, tnd_qsnow, rndst, nacon, &
  !$acc&              frzimm, frzcnt, frzdep, accre_enhan, pdel, re_ice, precip_frac ) &
  !$acc&     copyout( qcsevap, qisevap, qvres, vtrmc, vtrmi, qcsedten, qisedten, &
  !$acc&              qrsedten, qssedten, qgsedten, melttot, homotot, qcrestot, &
  !$acc&              qirestot, rflx, sflx, lflx, iflx, gflx, rercld, &
  !$acc&              tlat, qvlat, qctend, qitend, qstend, qrtend, nctend, nitend, &
  !$acc&              nrtend, nstend, qgtend, ngtend, prect, preci, ncal, ncai, &
  !$acc&              meltsdttot, frzrdttot, ums, umr, mnucct, umg, am_evp_st, qcrat, npsacwgtot, &
  !$acc&              nmultrgtot, nmultgtot, ngracstot, nscngtot, npracgtot, qmultrgtot, &
  !$acc&              qmultgtot, prdgtot, pgracstot, pgsacwtot, psacwgtot, pracgtot, psacrtot, &
  !$acc&              mnuccritot, mnuccrtot, pracstot, mnuccdtot, praitot, prcitot, &
  !$acc&              bergtot, bergstot, psacwstot, msacwitot, mnuccttot, mnuccctot, &
  !$acc&              prctot, pratot, qcsinksum_rate1ord, prodsnow, prain, prer_evap, &
  !$acc&              nevapr, evapsnow, cmeitot, cmeout, qrout, nrout, qsout, nsout, qgout, &
  !$acc&              ngout, effi, sadice, deffi, effc, lamcrad, pgamrad, effc_fn, sadsnow, &
  ...
  call qsat_water_all( mgncol, nlev, t(:,:), p(:,:), &
                       esl(:,:), qvl(:,:) )
  ...
  do k = 1, nlev
    do i = 1, mgncol
      ...
      if (t(i,k) > snowmelt .and. qs(i,k) > 0._r8) then
  ...
  if (trim(micro_mg_precip_frac_method) == 'in_cloud') then
   ...
   ! cloud liquid
   !-------------------------------------------
   call size_dist_param_liq_2D( mgncol, nlev,  mg_liq_props, qcic(:,:), ncic(:,:), & 
                                rho(:,:), pgam(:,:), lamc(:,:) )
   ...
   if (.not. do_sb_physics) then
     call kk2000_liq_autoconversion(mgncol, nlev, microp_uniform, qcic(:,:), &
                                    ncic(:,:), rho(:,:), relvar(:,:), &
                                    prc(:,:), nprc(:,:), nprc1(:,:) )
   endif
   ...
   call size_dist_param_basic_2D( mgncol, nlev, mg_ice_props, qiic(:,:), niic(:,:), &
                                  lami(:,:), n0=n0i(:,:))
                            
   ! Alternative autoconversion 
   if (do_sb_physics) then
     call sb2001v2_liq_autoconversion(mgncol, nlev, pgam(:,:),qcic(:,:),ncic(:,:), &
          qric(:,:),rho(:,:),relvar(:,:),prc(:,:),nprc(:,:),nprc1(:,:))     
   endif
     ...
     if (.not. use_hetfrz_classnuc) then
       ...
       do k = 1, nlev
         do i = 1, mgncol
           ...
           if ( (qcic(i,k).ge.qsmall) .and. (t(i,k).lt.269.15_r8) &
                  .and. (nnuccc(i,k)*lcldm(i,k).gt.nnuccd(i,k)) ) then
       ...
       call contact_freezing_2D(mgncol, nlev, mdust, microp_uniform, t(:,:), p(:,:), rndst(:,:,:), &
   ...
   call snow_self_aggregation_2D(mgncol, nlev, t(:,:), rho(:,:), asn(:,:), rhosn, & 
                                 qsic(:,:), nsic(:,:), nsagg(:,:) )

   call accrete_cloud_water_snow_2D(mgncol, nlev, t(:,:), rho(:,:), asn(:,:), uns(:,:), mu(:,:), &
   ...
   call accrete_rain_snow_2D(mgncol, nlev, t(:,:), rho(:,:), umr(:,:), ums(:,:), unr(:,:), uns(:,:), &
        qric(:,:), qsic(:,:), lamr(:,:), n0r(:,:), lams(:,:), n0s(:,:), &
        pracs(:,:), npracs(:,:))
        
   call heterogeneous_rain_freezing_2D(mgncol, nlev, t(:,:), qric(:,:), nric(:,:), lamr(:,:), &
                                       mnuccr(:,:), nnuccr(:,:))
                                       
   if (do_sb_physics) then
     call sb2001v2_accre_cld_water_rain_2D(mgncol, nlev, qcic(:,:), ncic(:,:), qric(:,:), &
                                           rho(:,:), relvar(:,:), pra(:,:), npra(:,:) )
   else
     call accrete_cloud_water_rain_2D(mgncol, nlev, microp_uniform, qric(:,:), qcic(:,:), &
                              ncic(:,:), relvar(:,:), accre_enhan(:,:), pra(:,:), npra(:,:) )
   endif
        
   call self_collection_rain_2D(mgncol, nlev, rho(:,:), qric(:,:), nric(:,:), nragg(:,:)) 
   ...
   if (do_cldice) then
      ...
      call ice_deposition_sublimation_2D(mgncol, nlev, t(:,:), q(:,:), qi(:,:), ni(:,:), &
   ...
   if(do_hail.or.do_graupel) then
     ...
     call graupel_collecting_snow_2D(mgncol, nlev, qsic(:,:),qric(:,:),umr(:,:),ums(:,:), &
           rho(:,:),lamr(:,:),n0r(:,:),lams(:,:),n0s(:,:), psacr(:,:))

     call graupel_collecting_cld_water_2D(mgncol, nlev, qgic(:,:),qcic(:,:),ncic(:,:),rho(:,:), &
           n0g(:,:),lamg(:,:),bgtmp,agn(:,:), psacwg(:,:), npsacwg(:,:))
             
     call graupel_riming_liquid_snow_2D(mgncol,nlev,psacws(:,:),qsic(:,:),qcic(:,:),nsic(:,:), &
          rho(:,:),rhosn,rhogtmp,asn(:,:),lams(:,:),n0s(:,:),deltat, &
          pgsacw(:,:),nscng(:,:))

     call graupel_collecting_rain_2D(mgncol,nlev,qric(:,:),qgic(:,:),umg(:,:), &
          umr(:,:),ung(:,:),unr(:,:),rho(:,:),n0r(:,:),lamr(:,:),n0g(:,:), &
          lamg(:,:), pracg(:,:),npracg(:,:))
     
!AG note: Graupel rain riming snow changes  
!    pracs, npracs, (accretion of rain by snow)  psacr (collection of snow by rain)

    call graupel_rain_riming_snow_2D(mgncol,nlev,pracs(:,:),npracs(:,:),psacr(:,:),qsic(:,:), &
          qric(:,:),nric(:,:),nsic(:,:),n0s(:,:),lams(:,:),n0r(:,:),lamr(:,:), &
          deltat,pgracs(:,:),ngracs(:,:))
       
    call graupel_rime_splintering_2D(mgncol,nlev,t(:,:),qcic(:,:),qric(:,:),qgic(:,:), &
          psacwg(:,:),pracg(:,:),qmultg(:,:),nmultg(:,:),qmultrg(:,:), &
          nmultrg(:,:))

    call evaporate_sublimate_precip_graupel_2D(mgncol,nlev,t(:,:), rho(:,:), &
          dv(:,:), mu(:,:), sc(:,:), q(:,:), qvl(:,:), qvi(:,:), &
          lcldm(:,:), precip_frac(:,:), arn(:,:), asn(:,:), agn(:,:), bgtmp, &
          qcic(:,:), qiic(:,:), qric(:,:), qsic(:,:), qgic(:,:), &
          lamr(:,:), n0r(:,:), lams(:,:), n0s(:,:), lamg(:,:), n0g(:,:), &
    ...
    call evaporate_sublimate_precip_2D(mgncol, nlev, t(:,:), rho(:,:), &
       dv(:,:), mu(:,:), sc(:,:), q(:,:), qvl(:,:), qvi(:,:), &
  ...
  do k = 1, nlev
    do i = 1, mgncol
      ...
      if ((pre(i,k)+prds(i,k)+prdg(i,k))*precip_frac(i,k)+ice_sublim(i,k) < -1.e-20_r8) then
         ...
         qtmp(i,k)=q(i,k)-(ice_sublim(i,k)+vap_dep(i,k)+mnuccd(i,k)+ &
              (pre(i,k)+prds(i,k)+prdg(i,k))*precip_frac(i,k))*deltat
              
         ttmp(i,k)=t(i,k)+((pre(i,k)*precip_frac(i,k))*xxlv+ &
     ...
     do i = 1, mgncol
       ...
       if ((pre(i,k)+prds(i,k)+prdg(i,k))*precip_frac(i,k)+ice_sublim(i,k) < -1.e-20_r8) then
         ...
         if (qtmp(i,k) > qvn(i,k)) then
            ...
            dum1_2D(i,k)=pre(i,k)*precip_frac(i,k)/((pre(i,k)+prds(i,k)+prdg(i,k))*precip_frac(i,k)+ice_sublim(i,k))
            ...
            ! modify rates if needed, divide by precip_frac to get local (in-precip) value
            prds(i,k) = dum_2D(i,k)*dum2_2D(i,k)/deltat/precip_frac(i,k)
            prdg(i,k) = dum_2D(i,k)*dum3_2D(i,k)/deltat/precip_frac(i,k)

            ! don't divide ice_sublim by cloud fraction since it is grid-averaged
            dum1_2D(i,k) = (1._r8-dum1_2D(i,k)-dum2_2D(i,k)-dum3_2D(i,k))
            ice_sublim(i,k) = dum_2D(i,k)*dum1_2D(i,k)/deltat
...
! Re-apply graupel freezing/melting
  ...
  do n = 1,nstep_max
      ...
      do k = 1, nlev
        do i=1,mgncol
          if( n <= nstep(i) ) then 
            ...
            faloutni(i,k) = fni(i,k) * dumni(i,k)
          end if
        end do
      end do
    else
    ...
    do i=1,mgncol
      if( n <= nstep(i) ) then
        ...
        faltndi = falouti(i,1)/pdel(i,1)
        faltndni = faloutni(i,1)/pdel(i,1)
        qitend(i,1) = qitend(i,1)-faltndi/nstep(i)
        nitend(i,1) = nitend(i,1)-faltndni/nstep(i)

        ! sedimentation tendency for output
        qisedten(i,1)=qisedten(i,1)-faltndi/nstep(i)

        dumi(i,1) = dumi(i,1)-faltndi*deltat/nstep(i)
        dumni(i,1) = dumni(i,1)-faltndni*deltat/nstep(i)
      end if
    end do
    ...
    do k = 2, nlev
      do i = 1, mgncol
        if( n <= nstep(i) ) then
          ...
          faltndqie=(falouti(i,k)-falouti(i,k-1))/pdel(i,k)
          faltndi=(falouti(i,k)-dum1*falouti(i,k-1))/pdel(i,k)
          faltndni=(faloutni(i,k)-dum1*faloutni(i,k-1))/pdel(i,k)
    ...
    do k = 1,nlev
      do i=1,mgncol
        if( n <= nstep(i) ) then
          iflx(i,k+1) = iflx(i,k+1) + falouti(i,k) / g / real(nstep(i))
        end if
      end do
    end do
    ...
    do i=1,mgncol
      if( n <= nstep(i) ) then
        ...
        preci(i) = preci(i)+falouti(i,nlev)/g/real(nstep(i))/1000._r8
      end if
    end do
   ...
   do n = 1,nstep_max
     ...
     do k = 1, nlev
       do i=1,mgncol
         if( n <= nstep(i) ) then 
           ...
           faloutnc(i,k) = fnc(i,k) * dumnc(i,k)
         end if
       end do
     end do
     ...
     do i=1,mgncol
       if( n <= nstep(i) ) then
         ! add fallout terms to microphysical tendencies
         faltndc = faloutc(i,1)/pdel(i,1)
         faltndnc = faloutnc(i,1)/pdel(i,1)
         qctend(i,1) = qctend(i,1)-faltndc/nstep(i)
         nctend(i,1) = nctend(i,1)-faltndnc/nstep(i)

         ! sedimentation tendency for output
         qcsedten(i,1)=qcsedten(i,1)-faltndc/nstep(i)

         dumc(i,1) = dumc(i,1)-faltndc*deltat/nstep(i)
         dumnc(i,1) = dumnc(i,1)-faltndnc*deltat/nstep(i)
       end if
     end do
     ...
     do k = 2,nlev
       do i=1,mgncol
         if( n <= nstep(i) ) then
           ...
           dum=min(dum,1._r8)
           faltndqce=(faloutc(i,k)-faloutc(i,k-1))/pdel(i,k)
           faltndc=(faloutc(i,k)-dum*faloutc(i,k-1))/pdel(i,k)
           faltndnc=(faloutnc(i,k)-dum*faloutnc(i,k-1))/pdel(i,k)
    ...
    do k = 1,nlev
      do i=1,mgncol
        if( n <= nstep(i) ) then
           lflx(i,k+1) = lflx(i,k+1) + faloutc(i,k) / g / real(nstep(i))
        end if
      end do
    end do
  ...
  do n = 1, nstep_max
    ...
    do k = 1, nlev
      do i=1,mgncol
        if( n <= nstep(i) ) then 
          ...
          faloutnr(i,k) = fnr(i,k) * dumnr(i,k)
        end if
      end do
    end do
    ...
    do i=1,mgncol
      if( n <= nstep(i) ) then 
        ! add fallout terms to microphysical tendencies
        faltndr = faloutr(i,1)/pdel(i,1)
        faltndnr = faloutnr(i,1)/pdel(i,1)
        qrtend(i,1) = qrtend(i,1)-faltndr/nstep(i)
        nrtend(i,1) = nrtend(i,1)-faltndnr/nstep(i)

        ! sedimentation tendency for output
        qrsedten(i,1)=qrsedten(i,1)-faltndr/nstep(i)

        dumr(i,1) = dumr(i,1)-faltndr*deltat/real(nstep(i))
        dumnr(i,1) = dumnr(i,1)-faltndnr*deltat/real(nstep(i))
      end if
    end do
    ...
    do k = 2,nlev
      do i=1,mgncol
        if( n <= nstep(i) ) then 
           ...
           faltndr=(faloutr(i,k)-faloutr(i,k-1))/pdel(i,k)
           faltndnr=(faloutnr(i,k)-faloutnr(i,k-1))/pdel(i,k)
     ...
     do k = 2,nlev
       do i = 1,mgncol
         if( n <= nstep(i) ) then 
           rflx(i,k+1) = rflx(i,k+1) + faloutr(i,k) / g / real(nstep(i))
         end if
       end do
     end do
   ...
   do n = 1, nstep_max
     ...
     do k = 1, nlev
       do i=1,mgncol
         if( n <= nstep(i) ) then
           falouts(i,k)  = fs(i,k)  * dums(i,k)
           faloutns(i,k) = fns(i,k) * dumns(i,k)
         end if
       end do 
     end do
     ...
     do i=1,mgncol
       if( n <= nstep(i) ) then     
         ...
         ! add fallout terms to microphysical tendencies
         faltnds = falouts(i,1)/pdel(i,1)
         faltndns = faloutns(i,1)/pdel(i,1)
         qstend(i,1) = qstend(i,1)-faltnds/nstep(i)
         nstend(i,1) = nstend(i,1)-faltndns/nstep(i)

         ! sedimentation tendency for output
         qssedten(i,1)=qssedten(i,1)-faltnds/nstep(i)

         dums(i,1) = dums(i,1)-faltnds*deltat/real(nstep(i))
         dumns(i,1) = dumns(i,1)-faltndns*deltat/real(nstep(i))
       end if
     end do
     ...
     do k = 2,nlev
       do i=1,mgncol
         if( n <= nstep(i) ) then 
           ...
           faltnds=(falouts(i,k)-falouts(i,k-1))/pdel(i,k)
           faltndns=(faloutns(i,k)-faloutns(i,k-1))/pdel(i,k)
     ...
     do k = 2,nlev
       do i = 1,mgncol
         if( n <= nstep(i) ) then
           sflx(i,k+1) = sflx(i,k+1) + falouts(i,k) / g / real(nstep(i))
         end if
       end do
     end do
     ...
     do i = 1,mgncol
       if( n <= nstep(i) ) then
         ...
         preci(i) = preci(i)+falouts(i,nlev)/g/real(nstep(i))/1000._r8
       end if
     end do
   ...
   do n = 1, nstep_max
     ...
     do k = 1, nlev
       do i=1,mgncol
         if( n <= nstep(i) ) then
           ...
           faloutng(i,k) = fng(i,k) * dumng(i,k)
         end if
       end do
     end do
     ...
     do i=1,mgncol
       if( n <= nstep(i) ) then     
         ! add fallout terms to microphysical tendencies
         faltndg = faloutg(i,1)/pdel(i,1)
         faltndng = faloutng(i,1)/pdel(i,1)
         qgtend(i,1) = qgtend(i,1)-faltndg/nstep(i)
         ngtend(i,1) = ngtend(i,1)-faltndng/nstep(i)

         ! sedimentation tendency for output
         qgsedten(i,1)=qgsedten(i,1)-faltndg/nstep(i)

         dumg(i,1) = dumg(i,1)-faltndg*deltat/real(nstep(i))
         dumng(i,1) = dumng(i,1)-faltndng*deltat/real(nstep(i))
       end if
     end do
     ...
     do k = 2,nlev
       do i=1,mgncol
         if( n <= nstep(i) ) then 
           ...
           faltndg=(faloutg(i,k)-faloutg(i,k-1))/pdel(i,k)
           faltndng=(faloutng(i,k)-faloutng(i,k-1))/pdel(i,k)
     ...
     do k = 2,nlev
       do i = 1,mgncol
         if( n <= nstep(i) ) then
           gflx(i,k+1) = gflx(i,k+1) + faloutg(i,k) / g / real(nstep(i))
         end if
       end do 
     end do
     ...
     do i = 1,mgncol
       if( n <= nstep(i) ) then
         ...
         preci(i) = preci(i)+faloutg(i,nlev)/g/real(nstep(i))/1000._r8
       end if
     end do
     ...
     do k=1,nlev
        do i=1,mgncol
           if (qtmp(i,k) > qvn(i,k) .and. qvn(i,k) > 0 .and. allow_sed_supersat) then
              ...
              if (ttmp(i,k) > 268.15_r8) then
                 dum1=0.0_r8
                 ! now add to tendencies, partition between liquid and ice based on te
                 !-------------------------------------------------------
              else if (ttmp(i,k) < 238.15_r8) then
                 dum1=1.0_r8
              else
                 dum1=(268.15_r8-ttmp(i,k))/30._r8
              end if

              dum = (qtmp(i,k)-qvn(i,k))/(1._r8+(xxls*dum1+xxlv*(1._r8-dum1))**2 &
                   *qvn(i,k)/(cpp*rv*ttmp(i,k)**2))/deltat
...
subroutine calc_rercld(mgncol, nlev, lamr, n0r, lamc, pgam, qric, qcic, ncic, rercld)
  ...
  do k = 1, nlev
    do i = 1, mgncol
      ...
      if (lamr(i,k) > 0._r8) then
        Atmp = n0r(i,k) * pi / (2._r8 * lamr(i,k)**3._r8)
      else
        Atmp = 0._r8
      end if
      ...
      if (lamc(i,k) > 0._r8) then
        Atmp = Atmp + &
             ncic(i,k) * pi * rising_factorial(pgam(i,k)+1._r8, 2)/(4._r8 * lamc(i,k)**2._r8)
      end if

      if (Atmp > 0._r8) then
        rercld(i,k) = rercld(i,k) + 3._r8 *(qric(i,k) + qcic(i,k)) / (4._r8 * rhow * Atmp)
      end if
    end do
  end do
